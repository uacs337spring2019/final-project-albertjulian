...
(function(){
	...
	*/
	function getUsers(username, password){
		url = "https://lowes-inventory1.herokuapp.com"
		  fetch(url)
         .then(checkStatus)
	...
	function addNewPallete(){
		...
		const fetchOptions = {
         ...
         body : JSON.stringify(pallete)
		  ...
		  let url = "https://lowes-inventory1.herokuapp.com";
	...
	function removeItem(event){
		...
		if (radioRemove){
			if (confirm("ARE YOU SURE YOU WANT TO REMOVE THIS PALLETE!")) {
				...
				url = "https://lowes-inventory1.herokuapp.com?remove=true" 
				  fetch(url)
				 .then(checkStatus)
				 .then(function(responseText){
					let aisles = document.getElementsByClassName("aisle-row");
							...
							const pallete = {string: result1};
								const fetchOptions = {
								 ...
								 body : JSON.stringify(pallete)
								  ...
								  let url = "https://lowes-inventory1.herokuapp.com?remove=true";
								  fetch(url, fetchOptions)
									 .then(checkStatus)
									 .then(function(responseText) {
										console.log(responseText);
